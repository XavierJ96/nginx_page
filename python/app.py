...
def check_health():
    try:
        ...
        conn = psycopg2.connect(**db_params)
        cur = conn.cursor()
        if not conn or not cur:  # If connection failed
            return (
        ...
        cur.execute(
            "SELECT 1;"
        )  # Execute any simple SQL command to confirm database access
        # If connection was successful, return a JSON object indicating this
        return (
            jsonify({"status": "OK", "connected": True}),
            200,
        )  # Return a success response
    except Exception as e:
        # Handle any exceptions during database access
        conn.rollback()  # Roll back any changes made to the connection if an exception occurred
        cur.close()  # Close the cursor
        return (
            jsonify(
                {
                    "error": "An error occurred while attempting to connect to the database."
                }
            ),
            500,
        )
